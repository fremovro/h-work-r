...
const generateAccessToken = (userData, remember) => {
  // expires after half and hour (1800 seconds = 30 minutes, 259200 seconds = 30 days)
  let time;
  remember ? time = '259200s' : time = '1800 s';
  return jwt.sign(userData, secretKey, { expiresIn: time });
}
...
server.post('/token', function (req, res) {
  ...
  if (user) {
    const token = generateAccessToken({ email: user.email }, remember);
    res.json({ token });
  }
  else {
